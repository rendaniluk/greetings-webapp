...
module.exports = function(models) {
    ...
    const generateGreetings = function(req, res, next) {
        ...
        if ((!person || !person.name) || !radioBtn) {
          req.flash('error', 'Please enter name and select radio button!')
          res.redirect('/')
        } else {
          models.greeted.create(person, function(err, results) {
            if (err) {
              if (err.code === 11000) {
    ...
    const counter = function(req, res, next) {
        ...
        models.greeted.findOne({
          ...
          .exec(function(err, greetMap) {
            if (err) {
              return next(err);
            } else {
              var countEach = greetMap.count
              var greetingCounterMsg = 'Hello, ' + person +
                ' has been greeted ' +
                countEach + ' time(s)'
